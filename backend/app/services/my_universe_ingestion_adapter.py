"""MY market universe ingestion adapter with deterministic canonicalization.

Malaysia lists on Bursa Malaysia (Main Market + ACE Market; MIC: XKLS). Bursa
issuer codes are four-digit numerics (1155, 1295, 5347) that Yahoo Finance
prefixes onto the ``.KL`` suffix. This adapter normalizes KLSE-style and
Yahoo-style inputs to ``<NNNN>.KL`` and emits a single deterministic row per
canonical symbol.
"""
...
_MY_EXCHANGE_ALIASES: dict[str, str] = {
    "KLSE": "XKLS",
    "MYX": "XKLS",
    "XKLS": "XKLS",
    "BURSA": "XKLS",
...
# Sources that are considered policy-approved for MY universe ingestion.
_APPROVED_MY_SOURCES: frozenset[str] = frozenset(
    {
        "bursa_official",
        "my_manual_csv",
        "my_reference_bundle",
...
# Bursa Malaysia listing codes are exactly four numeric digits.
_MY_LOCAL_CODE_RE = re.compile(r"^[0-9]{4}$")


@dataclass(frozen=True)
class MYCanonicalUniverseRow:
    """Canonical MY row emitted by the ingestion adapter."""
...
@dataclass(frozen=True)
class MYRejectedUniverseRow:
    """Rejected MY row with reason."""
...
@dataclass(frozen=True)
class MYCanonicalizationResult:
    """Canonicalization output, split into accepted and rejected rows."""

    canonical_rows: tuple[MYCanonicalUniverseRow, ...]
    rejected_rows: tuple[MYRejectedUniverseRow, ...]


class MYUniverseIngestionAdapter:
    """Normalize and validate MY universe rows for deterministic snapshots."""
    ...
    def is_approved_source(cls, source_name: str) -> bool:
        ...
        if normalized in _APPROVED_MY_SOURCES:
            return True
        return normalized.startswith("bursa_") or normalized.startswith("my_")
    ...
    def _normalize_exchange(raw_exchange: Any) -> str:
        exchange = str(raw_exchange or "").strip().upper() or "XKLS"
        normalized = _MY_EXCHANGE_ALIASES.get(exchange)
        if normalized is None:
            raise ValueError(
                f"Unsupported MY exchange '{exchange}'. Expected one of: "
                "KLSE, MYX, XKLS, BURSA"
            )
        return normalized

    @staticmethod
    def _normalize_my_local_code(source_symbol: str) -> str:
        token = source_symbol
        for prefix in ("KLSE:", "MYX:", "XKLS:", "BURSA:"):
        ...
        if not _MY_LOCAL_CODE_RE.fullmatch(token):
            raise ValueError(
                f"Invalid MY symbol '{source_symbol}'. "
                "Expected 4-digit Bursa local code with optional .KL suffix."
            )
        return token
    ...
    @staticmethod
    def _selection_key(row: MYCanonicalUniverseRow) -> tuple[str, int]:
    ...
    def _merge_duplicate_rows(
        self,
        first: MYCanonicalUniverseRow,
        second: MYCanonicalUniverseRow,
    ) -> MYCanonicalUniverseRow:
    ...
    def canonicalize_rows(
        ...
        *,
        ...
        canonical_by_symbol: dict[str, MYCanonicalUniverseRow] = {}
        rejected_rows: list[MYRejectedUniverseRow] = []
        ...
        for index, raw_row in enumerate(rows, start=1):
            ...
            try:
                ...
                local_code = self._normalize_my_local_code(source_symbol)
                ...
                canonical_exchange = identity.exchange or "XKLS"
                canonical_payload = {
                ...
                canonical_row = MYCanonicalUniverseRow(
                    symbol=identity.canonical_symbol,
                    name=row_name,
                    market=identity.market,
                    exchange=canonical_exchange,
        ...
        return MYCanonicalizationResult(
            canonical_rows=canonical_rows,
            rejected_rows=rejected_rows_tuple,
        )


my_universe_ingestion_adapter = MYUniverseIngestionAdapter()
